...
const moviesController = {
  ...
  search: async (req, res) => {
    const resultado = await db.Movie.findAll({
      where: {
        title: {
    ...
    if (resultado.length > 0) {
      ...
      fetch("http://www.omdbapi.com/?apikey=3d8f67ad&s=" + req.body.titulo)
        .then((omdbApiResponse) => {
          return omdbApiResponse.json();
        })
        .then((omdbApiMovies) => {
          let result = [];

          if (omdbApiMovies.Response === "True") {
            result = omdbApiMovies.Search.map((omdbApiMovie) => {
              return {
                id: omdbApiMovie.imdbID,
